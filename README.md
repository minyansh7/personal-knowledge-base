...
- **git** — versions schema, skills, and scripts; wiki content stays local

## Authentication

Background ingest runs on Claude subscription (no API key). Auth is handled via a long-lived OAuth token stored outside the repo:

```
~/.claude_ingest_token   ← generated by: claude setup-token
                         ← never committed to git
```

`ingest.sh` reads this file and sets `CLAUDE_CODE_OAUTH_TOKEN` before calling `claude --print`. `ANTHROPIC_API_KEY` is explicitly unset so API billing never occurs.

> **Watch out:** The long-lived token will eventually expire. When ingest starts failing with `401` errors in `scripts/watcher.log`, run `claude setup-token` in your terminal, save the output to `~/.claude_ingest_token`, and you're done.
...
Wiki content, raw sources, assets, and config are local only — not pushed.

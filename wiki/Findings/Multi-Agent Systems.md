...
# Going Multi-Agent Is a Coordination Cost Decision, Not a Capability One

The reason most multi-agent systems fail in production isn't that agents reason poorly — it's that the cost of coordinating them exceeds the value of parallelizing them, and this calculation is almost never done upfront.

Read-heavy tasks (research, retrieval, analysis) have low coordination cost because outputs are independent — agents work in parallel without negotiating shared state. Write-heavy tasks (coding, drafting, editing) have high coordination cost because outputs must be merged and conflicting decisions compound. This asymmetry is systematically hidden in demos, which almost universally use research tasks. The field's apparent progress on multi-agent systems is benchmarked on the easy half of the problem.

The right first question is not "can I break this into parallel tasks?" It is "what is the coordination overhead when those tasks produce conflicting outputs, and does the parallelization gain exceed it?" For write-heavy work, the answer is almost always no — until someone specifically solves the merge problem.
...
- [[Findings/Context as Maintenance]] — How well context contracts between agents are specified determines coordination cost directly.
